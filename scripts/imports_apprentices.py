...
def import_apprentices(file_path):
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            cursor.execute("SELECT id FROM users WHERE identity_document = %s", (str(row['NUMERO_DOCUMENTO']),))
            user = cursor.fetchone()
            if not user:
                cursor.execute(
                    """INSERT INTO users (name, last_name, identity_document, email, document_type_id)
                    VALUES (%s, %s, %s, %s, %s)""",
                    (
                        row['NOMBRE'],
                        f"{row['PRIMER_APELLIDO']} {row['SEGUNDO_APELLIDO']}".strip(),
            ...
            # --- 3. Obtener Training Center ID ---
            cursor.execute("SELECT id FROM training_centers WHERE code = %s", (str(row['ID_CENTRO']),))
            training_center = cursor.fetchone()
            if not training_center:
                print(f"Error: No se encontró el centro con código {row['ID_CENTRO']}")
                continue
            training_center_id = training_center[0]

            # --- 4. Obtener Course ---
            cursor.execute("SELECT id FROM courses WHERE code = %s", (str(row['FICHA']),))
            course = cursor.fetchone()
            if not course:
                print(f"Error: No se encontró la ficha {row['FICHA']}")
                continue
            ...
            # --- 5. Insertar en Apprentices ---
            estado = str(row['ESTADO_APRENDIZ']).strip() if pd.notnull(row['ESTADO_APRENDIZ']) else 'Formacion'
            state = state_mapping.get(estado, 'Formacion')
            ...
            # --- 6. Insertar en role_training_center_user ---
            cursor.execute(
                """SELECT id FROM role_training_center_user
                WHERE user_id = %s AND role_id = 4 AND training_center_id = %s""",
